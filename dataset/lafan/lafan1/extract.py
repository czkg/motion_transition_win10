...
def get_lafan1_seq(in_path):
    """
    Extract all the sets
    Params:
        in_path: path to a sequence in dataset BVH
    """
    X = []       #local position
    Q = []       #local quaternion
    rv = []      #root velocity

    anim = read_bvh(in_path)

    # extract each frame
    for i in range(anim.pos.shape[0]):
        X.append(anim.pos[i])
        Q.append(anim.quats[i])

        q, x = utils.quat_fk(anim.quats[i], anim.pos[i], anim.parents)
        rv.append(x[0])
