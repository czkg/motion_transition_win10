...
# Orient the data according to the last past keframe
...
def quat2angvel(x_xt):
    """get angular velocity from quaternion
    parameter:
        x_xt: quaternion and derivative of quaternion of shape (8,)
    """
    x = x_xt[...,:4]
    xt = x_xt[...,4:]
    q = quaternion.from_float_array(x)
    qt = quaternion.from_float_array(xt)
    ang_vel = quaternion.as_float_array(2*qt/q)[1:]
    return ang_vel


def compute_force_ang_nt(ang_a, x_local, mass=1.):
    """compute force using Newton's second law (F=ma)
    Inputs:
        a: angular accelerations
        x_local: local positions
        m: mass
    Output:
    ...
    r[:,0] = 0.
    #broadcast r to have the shape of ang_a
    r = np.expand_dims(r, axis=-1)
    r = np.concatenate((r,r,r), axis=-1)

    #linear acceleration a = r * alpha, where alpha is angular acceleration
    lin_a = np.multiply(ang_a, r)

    mass = np.full(lin_a.shape, mass)

    #compute force
    force = np.multiply(lin_a, mass)
    return force, lin_a


def compute_force_lin_nt(lin_a, mass=1.):
    """compute force using Newton's second law (F=ma)
    Inputs:
